...
if (confirm --title "ibus" --yesno "Would you like to install ibus for improved non-Latin input via iBus?" 8 65) then
  ...
  if (confirm --title "ibus daemon" --yesno "Would you like iBus daemon to run each time you open Pengwin? WARNING: Requires an X server to be running or it will generate errors." 9 70) then
