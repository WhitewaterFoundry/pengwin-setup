...
function test_py_poetry() {
  ...
  assertEquals "Python was not installed" "1" "$(run python3 --version | grep -c '^Python 3\.13')"
  assertEquals "Poetry was not installed" "1" "$(run "${HOME}"/.local/bin/poetry --version | grep -c -E 'version [0-9]+\.[0-9]+\.[0-9]+')"
}
