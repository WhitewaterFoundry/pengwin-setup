...
function test_main() {
  ...
  assertEquals "Brew was not installed" "1" "$(run brew --version | grep -c -E 'Homebrew [0-9]+')"
}
...
function test_uninstall() {
  ...
  assertEquals "Brew was still installed after uninstall" "0" "$(run brew --version | grep -c -E 'Homebrew [0-9]+')"
}
