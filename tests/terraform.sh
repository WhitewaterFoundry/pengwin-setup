...
function test_main() {
  ...
  assertNotEquals "Terraform was not installed" "0" "$(run /usr/bin/terraform --version | grep -c '1.13')"
  assertNotEquals "Terraform needs to be updated in the installer," "2" "$(run /usr/bin/terraform --version | grep -c '1.13')"
}
