...
function test_main() {
  ...
  assertEquals "nodejs latest was not installed" "1" "$(run node --version | grep -c 'v20')"
  assertEquals "nodejs lts was not installed" "1" "$(run n list | grep -c 'node/18')"
...
function test_uninstall() {
  ...
  assertEquals "nodejs latest was not uninstalled" "0" "$(run node --version | grep -c 'v20')"
  assertEquals "nodejs lts was not uninstalled" "0" "$(run n list | grep -c 'node/18')"
