...
function testMain() {
  ...
  #assertEquals "Fish is not the default shell" "/usr/bin/fish" "$(grep $USER </etc/passwd | cut -f 7 -d ":")"
...
function testUninstall() {
  ...
  #assertEquals "Bash is not the default shell" "/usr/bin/bash" "$(grep $USER </etc/passwd | cut -f 7 -d ":")"
